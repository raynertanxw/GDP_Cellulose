...
public class EMHelper : MonoBehaviour 
{
	...
	void FixedUpdate ()
	{
		...
		if (fMinToMaxYRatio != (transform.position.y - Constants.s_fEnemyMainMinY) / (Constants.s_fEnemyMainMaxY - Constants.s_fEnemyMainMinY))
			fMinToMaxYRatio = (transform.position.y - Constants.s_fEnemyMainMinY) / (Constants.s_fEnemyMainMaxY - Constants.s_fEnemyMainMinY);
