	...
	public void SetEndGamePanelVisibility(bool _visible)
	{
		if (_visible)
		{
			if (GameManager.Instance.bPlayerWon == true)
			{
				...
				AudioManager.PlayPMSoundEffect(PlayerMainSFX.Win);
			}
			else
			{
				SetWinGameVisibility(false);
				SetLoseGameVisibility(true);
				AudioManager.PlayPMSoundEffect(PlayerMainSFX.MainDeath);
				AudioManager.PlayPMSoundEffect(PlayerMainSFX.Lose);
			}
