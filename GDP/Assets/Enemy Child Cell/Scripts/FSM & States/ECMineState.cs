...
public class ECMineState : IECState {
	...
	{
		float Range = 4f;
		Debug.Log(Vector2.Distance(m_Child.transform.position,m_ecFSM.m_PMain.transform.position));
		if(Vector2.Distance(m_Child.transform.position,m_ecFSM.m_PMain.transform.position) <= Range)
		{
			return true;
		}
		return false;
